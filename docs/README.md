...
| [Product Overview](product-overview.md) | Core concept, terminology, season lifecycle, and the features added since the first build |
...
| [Mock season](mock-season.md) | Playing a whole season in minutes against a fake ESPN, for testing settlement end to end |
| [Input: user story](inputs/user-story.md) | The source note behind Spread Sharks, with the decisions taken |
| [Input: commissioner controls](inputs/commissioner.md) | Scoping what a commissioner can do pre-season, midseason, and post-season |
...
Everything described across these documents is implemented. The features added
after the first working build — college football, gated pool creation, standing
in the pool view, bet-history filters — are covered in
[Product Overview](product-overview.md#new-features), with the feed measurements
behind the second league in [Data Sources](data-sources.md) and the schema and
defect history in [MVP Implementation](mvp.md#the-league-dimension).
