...
The cap is on **aggregate exposure to one selection** — one side of one market on
one game — not on each individual wager. So a member may add to a position across
several bets, at whatever lines were posted when each was struck, until that
selection reaches the cap. Backing the other side, a different market, or a
different game each opens a fresh allowance.
