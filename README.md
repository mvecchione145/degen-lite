...
The database bootstraps only what you need to sign in: a single account, `admin`
(password `password123`), granted permission to create pools. No pools and no
sports data — the schedule arrives from ESPN when the worker starts.
...
Either schedule can hold several `;`-separated cron expressions, so a feed can
run on a reduced overnight cadence — see [`docs/mvp.md`](docs/mvp.md#configuration).
